...
exit;
	...
	if ($tipo!=0){
    mysql_query("INSERT INTO orcamento (mes,ano,valor,conta,usuario) values ('$mes','$ano','$valor_orcamento','1','$usuario')");
    echo mysql_error();
...
<h2 style="margin:5px"><a href=index.php style="color:#FFF">Conta Corrente</a> | <a href=visa.php style="color:#008B8B">Cartão Visa</a> | <a href=master.php style="color:#008B8B">Cartão Master</a></h2>
</td>
...
<table cellpadding="5" cellspacing="0" width="1000" align="center">
<?php
$qr=mysql_query("SELECT SUM(valor) as total FROM orcamento WHERE mes='$mes_hoje' && ano='$ano_hoje' && conta=1 && usuario='$usuario'");
...
$qra=mysql_query("SELECT SUM(valor) as total FROM orcamento WHERE ano='$ano_hoje' && conta=1 && usuario='$usuario'");
$rowa=mysql_fetch_array($qra);
$total_ano=$rowa['total'];

$qra=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && ano='$ano_hoje'");
$rowa=mysql_fetch_array($qra);
...
$qrc=mysql_query("SELECT SUM(valor) as total FROM orcamento WHERE mes='$mes_hoje' && ano='$ano_hoje' && conta>1 && usuario='$usuario'");
$rowc=mysql_fetch_array($qrc);
$total_cart=$rowc['total'];

$qrc=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta>1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$rowc=mysql_fetch_array($qrc);
...
<td>
<a href="javascript:;" style="font-size:12px" title="Orçamento anual">Orçamento anual: <?php echo formata_dinheiro($gasto_ano)?><?php echo " de "?><?php echo formata_dinheiro($total_ano)?><?php echo " resta "?><font color="<?php if ($resta_ano <= 0) echo "#C00"?>"><?php echo formata_dinheiro($resta_ano)?></font>.</a><br>
<div class="outter2">
...
<td>
<a href="javascript:;" style="font-size:12px" title="Orçamento mensal cartões">Orçamento mensal cartões: <?php echo formata_dinheiro($gasto_cart)?><?php echo " de "?><?php echo formata_dinheiro($total_cart)?><?php echo " resta "?><font color="<?php if ($resta_cart <= 0) echo "#C00"?>"><?php echo formata_dinheiro($resta_cart)?></font>.</a><br>
<div class="outter3">
...
?>
    ...
    <fieldset>
        ...
        <table cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td><br><span style="font-size:18px; color:#000">Saídas:</span></td>
...
<tr>
<td><br><span style="font-size:19px; color:#000">Saídas:</span></td>
...
<td align="right">
<?php
$qrconta2=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qrconta2);
$conta2=$row['total'];

$qrconta3=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=3 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qrconta3);
$conta3=$row['total'];

$totalcontascart=$conta2+$conta3
?>
<b>Faturas de cartões de crédito em aberto:</b> <span style="font-size:16px; color:#C00"><?php echo formata_dinheiro($totalcontascart)?></span>
</td>
</tr>
</table>
...
if (isset($_GET['filtro_cat'])){
	if ($_GET['filtro_cat']!=''){	
		$filtros="&& cat='".$_GET['filtro_cat']."'";
                ...
                $qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $entradas=$row['total'];

                $qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=1 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $saidas=$row['total'];
