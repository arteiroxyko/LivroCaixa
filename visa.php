...
exit();
	...
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','1','$dataed','2','$usuario')");
    echo mysql_error();}

	if ($mes!=$rowed['mes']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','2','$dataed','2','$usuario')");
    echo mysql_error();}
	
	if ($ano!=$rowed['ano']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','3','$dataed','2','$usuario')");
    echo mysql_error();}
	
	if ($tipo!=$rowed['tipo']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','4','$dataed','2','$usuario')");
    echo mysql_error();}
	
	if ($cat!=$rowed['cat']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','5','$dataed','2','$usuario')");
    echo mysql_error();}
	
	if ($descricao!=$rowed['descricao']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','6','$dataed','2','$usuario')");
    echo mysql_error();}
	
	if ($valor!=$rowed['valor']){
	mysql_query("UPDATE movimentos SET dia='$dia', mes='$mes', ano='$ano', tipo='$tipo', cat='$cat', conta='$conta_lan', descricao='$descricao', valor='$valor', edicao='Editado' WHERE id='$id'");
	mysql_query("INSERT INTO historico (id_mov,just_id,data,conta_mov,usuario) values ('$id','7','$dataed','2','$usuario')");
    echo mysql_error();}
...
exit();
	...
	$n=1;
	while ($n <= $parcelas) {	
    mysql_query("INSERT INTO movimentos (dia,mes,ano,tipo,descricao,valor,cat,conta,nparcela,parcelas,usuario) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat','2','$n','$parcelas','$usuario')");
    echo mysql_error();
...
if (isset($_POST['acao']) && $_POST['acao'] == 'limite_visa') {
    ...
    mysql_query("UPDATE usuarios SET visa='$limitevisa', dia_venc_v='$diavenc' WHERE id='$usuario'");
    echo mysql_error();
...
exit;
	...
	if ($tipo!=0){
	mysql_query("INSERT INTO orcamento (mes,ano,valor,conta,usuario) values ('$mes','$ano','$valor_orcamento','2','$usuario')");
    echo mysql_error();
    header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
	exit();
	}
...
exit();
	...
	if ($tipo!=0){
	mysql_query("UPDATE orcamento SET valor='$valor_orcamento' WHERE mes='$mes' && conta='2' && usuario='$usuario'");
    echo mysql_error();
	header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
    exit();
	}
    mysql_query("UPDATE orcamento SET valor='$valor_orcamento' WHERE mes>=$mes && ano='$ano' && conta='2' && usuario='$usuario'");
    echo mysql_error();
	header("Location: ?mes=" . $_GET['mes'] . "&ano=" . $_GET['ano']);
    exit();
}
...
<h2 style="margin:5px"><a href=index.php style="color:#008B8B">Conta Corrente</a> | <a href=visa.php style="color:#FFF">Cartão Visa</a> | <a href=master.php style="color:#008B8B">Cartão Master</a></h2>
</td>
...
<table cellpadding="5" cellspacing="0" width="1000" align="center">
<?php
$qr=mysql_query("SELECT SUM(valor) as total FROM orcamento WHERE mes='$mes_hoje' && ano='$ano_hoje' && conta=2 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$total=$row['total'];

$qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qr);
$gasto=$row['total'];
$resta=$total-$gasto;
...
<?php
$qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje'");
$row=mysql_fetch_array($qr);
$entradas=$row['total'];
...
<?php
$qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=2 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$entradas=$row['total'];

$qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=2 && usuario='$usuario'");
$row=mysql_fetch_array($qr);
$saidas=$row['total'];

$qr=mysql_query("SELECT dia_venc_v, SUM(visa) as total FROM usuarios WHERE id='$usuario'");
$row=mysql_fetch_array($qr);
$valorvisa=$row['total'];
...
<td align="right"><span style="font-size:18px"><a href="javascript:;" style="font-size:16px; color:rgba(4, 45, 191, 1)" onclick="abreFecha('limite_visa');" title="Alterar limite"><?php echo $visa?></a></span></td>
</tr>
...
<b>Dia vencimento:</b> <input type="text" value="<?php echo $dia_venc_v?>" name="diavenc" id="diavenc" size="1" maxlength="2">
<input type="submit" class="input" value="Gravar" />
</form>
</td>
</tr>
...
<?php
$qr=mysql_query("SELECT DISTINCT c.id, c.nome FROM categorias c, movimentos m WHERE m.cat=c.id && c.usuario='$usuario' && m.mes=$mes_hoje && m.ano=$ano_hoje && m.conta=2 ORDER BY c.nome");
while ($row=mysql_fetch_array($qr)){
?>
...
if (isset($_GET['filtro_cat'])){
	if ($_GET['filtro_cat']!=''){	
		$filtros="&& cat='".$_GET['filtro_cat']."'";
                ...
                $qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=1 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $entradas=$row['total'];

                $qr=mysql_query("SELECT SUM(valor) as total FROM movimentos WHERE tipo=0 && conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros");
                $row=mysql_fetch_array($qr);
                $saidas=$row['total'];
...
$qr=mysql_query("SELECT * FROM movimentos WHERE conta=2 && usuario='$usuario' && mes='$mes_hoje' && ano='$ano_hoje' $filtros ORDER By dia");
$cont=0;
while ($row=mysql_fetch_array($qr)){
$cont++;
